...
@interface ListRootViewController ()<DOPDropDownMenuDataSource,DOPDropDownMenuDelegate,UISearchBarDelegate>
...
-(void)addNewWork{
   ...
   _newWorkImage=[[UIImageView alloc]initWithFrame:CGRectMake(SCREEN_WIDTH-85, SCREEN_HEIGHT-100, 60, 60)];
    _newWorkImage.image=[UIImage imageNamed:@"招聘"];
    _newWorkImage.userInteractionEnabled=YES;
    UIPanGestureRecognizer*pan=[[UIPanGestureRecognizer alloc]initWithTarget:self action:@selector(move:)];
    pan.minimumNumberOfTouches = 1;
    pan.maximumNumberOfTouches = 1;
    [_newWorkImage addGestureRecognizer:pan];
    UITapGestureRecognizer*tap=[[UITapGestureRecognizer alloc]initWithTarget:self action:@selector(publishNewWork)];
    tap.numberOfTapsRequired=1;
    tap.numberOfTouchesRequired=1;
    [_newWorkImage addGestureRecognizer:tap];
    [self.view addSubview:_newWorkImage];
...
-(void)initDict{
    ...
    [_parent removeObjectForKey:@"firstLocation"];
    [_parent removeObjectForKey:@"secordLocation"];
    [_parent removeObjectForKey:@"thirdLocation"];
    if ([model.name isEqualToString:@"全国"]==YES) {
    }else{
    
         [_parent setObject:[NSString stringWithFormat:@"%lu",model.id] forKey:valueString[_currentAdressArray.count-1]];
    }
...
-(void)requestInformation
{
    ...
    NSString*urlString=[self interfaceFromString:interface_list];
    [_parent setObject:[NSString stringWithFormat:@"%lu",_currentPage] forKey:@"pageNo"];
...
-(void)referCount{
    ...
    UILabel*label=(id)[self.view viewWithTag:500];
    label.text=[NSString stringWithFormat:@"没有更多了"];
