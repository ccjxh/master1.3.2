...
-(void)confirm{
    ...
    for (NSInteger i=0; i<_dataArray.count; i++) {
        ...
        if (model.isOwer==YES) {
            ...
            [dict setObject:[NSString stringWithFormat:@"%lu",model.id] forKey:@"id"];
            [dict setObject:model.name forKey:@"name"];
            [Array addObject:dict];
        }
    }
    
    self.model.servicerSkills=Array;
//    [self.model.servicerSkills setArray:Array];
    if (self.skillArray) {
...
-(void)initData{
    ...
    for (NSInteger i=0; i<Array.count; i++) {
        ...
        for (NSInteger j=0; j<self.Array.count; j++) {
            ...
            [tempModel setValuesForKeysWithDictionary:self.Array[j]];
