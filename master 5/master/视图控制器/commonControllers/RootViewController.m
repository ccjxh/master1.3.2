...
-(void)dealloc{
    ...
    [[NSNotificationCenter defaultCenter]removeObserver:self name:@"showReason" object:nil];
}
...
//        self.edgesForExtendedLayout = UIRectEdgeNone;
    ...
    self.view.backgroundColor=COLOR(226, 228, 226, 1);
    ...
    [[NSNotificationCenter defaultCenter]addObserver:self selector:@selector(showReason:) name:@"showReason" object:nil];
    // Do any additional setup after loading the view.
}


-(void)showReason:(NSNotification*)nc{

    NSDictionary*dict=nc.userInfo;
    NSString*reason=[dict objectForKey:@"reason"];
    [self.view makeToast:reason duration:1 position:@"center"];
...
-(void)showIncreaImage{
    for (UIView*view in _increaseView.subviews) {
        if (view.tag==101) {
