...
-(void)initUI{
    ...
    if (self.type==1) {
        view=[[findWorkDetail alloc]initWithType:self.type];
        }
    }
    ...
    __weak typeof(findWorkDetailModel*)weakModel=detailModel;

    self.navigationController.navigationBar.tintColor = [UIColor whiteColor];
    view.deleBlock=^(NSInteger ID){
    
        [weakSelf requestTokenWithID:ID];
    ...
    view.apply=^(){
        [weakSelf flowShow];
        NSString*urlString=[weakSelf interfaceFromString:interface_apply];
        NSDictionary*dict=@{@"projectId":[NSString stringWithFormat:@"%lu",detailModel.id]};
        [[httpManager share]POST:urlString parameters:dict success:^(AFHTTPRequestOperation *Operation, id responseObject) {
            NSDictionary*dict=(NSDictionary*)responseObject;
            [weakSelf flowHide];

            if ([[dict objectForKey:@"rspCode"] integerValue]==200) {
                [weakSelf.view makeToast:@"报名成功" duration:1 position:@"center" Finish:^{
                    [weakSelf popWithnimation:weakSelf.navigationController];
                }];
            }else{
            
                [weakSelf.view makeToast:[dict objectForKey:@"msg"] duration:1 position:@"center"];
            }
        } failure:^(AFHTTPRequestOperation *Operation, NSError *error) {
            [weakSelf flowHide];
            [weakSelf.view makeToast:@"当前网络繁忙,请稍后重试" duration:1 position:@"center"];
        }];
...
-(void)deleWithID:(NSInteger)ID{
    ...
    NSString*urlString=[self interfaceFromString:interface_refuseFinfWoker];
    NSDictionary*dict=@{@"token":self.token,@"projectId":[NSString stringWithFormat:@"%lu",ID]};
    [[httpManager share]POST:urlString parameters:dict success:^(AFHTTPRequestOperation *Operation, id responseObject) {
        [self flowHide];
        NSDictionary*dict=(NSDictionary*)responseObject;
        if ([[dict objectForKey:@"rspCode"] integerValue]==200) {
        ...
        [self.view makeToast:@"操作成功" duration:1 position:@"center" Finish:^{
...
-(void)requestInformation{
    ...
    [[httpManager share]POST:urlstring parameters:dict success:^(AFHTTPRequestOperation *Operation, id responseObject) {
        ...
        if ([[dict objectForKey:@"rspCode"] integerValue]==200) {
            ...
            detailModel.RECREIT_TYPE=[[inforDict objectForKey:@"RECRUIT_TYPE"] integerValue];
...
-(void)customNavigation{
    ...
    if (_type==1) {
        if (detailModel.auditState==0||detailModel.auditState==1||detailModel.auditState==3) {
            return;
        }
    }
