        ...
        if ([type isEqualToString:@"public.image"]) {
            ...
            NSData *imageData = UIImageJPEGRepresentation(imageNew, 0.5);
                ...
                [[httpManager share]POST:urlString parameters:dict constructingBodyWithBlock:^(id<AFMultipartFormData> formData) {
                    ...
                    [model setValuesForKeysWithDictionary:[[dict objectForKey:@"entity"]objectForKey:@"attachment"]];
                    [self.model.certaionArray addObject:model];
                    [self request];
...
-(UITableViewCell*)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    Cell.contentImage.autoresizingMask = UIViewAutoresizingFlexibleWidth;
        ...
        [[httpManager share]POST:urlString parameters:dict success:^(AFHTTPRequestOperation *Operation, id responseObject) {
            ...
            if ([[dict objectForKey:@"rspCode"] intValue]==200) {
                [self.view makeToast:@"删除成功" duration:1 position:@"center" Finish:^{
                    ...
                    for (NSInteger i=0; i<self.model.certaionArray.count; i++) {
                        certificateModel*temp=self.model.certaionArray[i];
                        if (temp.id==model.id) {
                            [self.model.certaionArray removeObjectAtIndex:i];
                        }
