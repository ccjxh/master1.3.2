    ...
    if ([valueModel.name isEqualToString:@"面议"]==YES) {
        ...
        [[httpManager share]POST:urlString parameters:dict success:^(AFHTTPRequestOperation *Operation, id responseObject) {
            ...
            if ([[dict objectForKey:@"rspCode"] integerValue]==200) {
                [self flowHide];
                self.model.expectPay=44;
                NSDictionary*dict=@{@"id":@"0",@"name":@"面议"};
                self.model.payType=dict;
                [self popWithnimation:self.navigationController];
                if (_expectBlock) {
                    _expectBlock();
                }
...
-(void)confirm:(UIButton*)button{
    ...
    [[httpManager share]POST:urlString parameters:dict success:^(AFHTTPRequestOperation *Operation, id responseObject) {
        ...
        if ([[dict objectForKey:@"rspCode"] integerValue]==200) {
            ...
            NSDictionary*dict=@{@"id":[NSString stringWithFormat:@"%lu",valueModel.id],@"name":valueModel.name};
            self.model.payType=dict;
            self.model.expectPay=[_tx.text integerValue];
