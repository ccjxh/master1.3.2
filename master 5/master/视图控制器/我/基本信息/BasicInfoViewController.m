...
@interface BasicInfoViewController ()<PhotoDelegate>
{
...
-(void) tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    switch (indexPath.section)
    {
        ...
        case 1:
        {
            ...
            if (indexPath.row==0) {
                if ((model.personal==0&&model.personalState==0)||model.personalState==3) {
        ...
        case 3:
        {
            ...
            if ((model.personalState == 0||model.personalState==3)&&model.personal==0){
            ...
            if (selectIamge == nil)
            {
                ...
                [[httpManager share]POST:urlString parameters:dict constructingBodyWithBlock:^(id<AFMultipartFormData> formData) {
                    ...
                    if ([[dict objectForKey:@"rspCode"] integerValue]==200) {
                      ...
                      if ([[[dict objectForKey:@"entity"] objectForKey:@"user"] objectForKey:@"integrity"] ) {
                          model.integrity=[[[dict objectForKey:@"entity"] objectForKey:@"user"] objectForKey:@"integrity"] ;
                          [[dataBase share]updatePrimaryInforWithModel:model];
                          }

                        if ([[[[dict objectForKey:@"entity"] objectForKey:@"user"] objectForKey:@"integral"] integerValue]!=0) {
                            
                                if ([model.integral integerValue]-[[[[dict objectForKey:@"entity"] objectForKey:@"user"] objectForKey:@"integral"] integerValue]<0) {
                                    model.integral=[[[dict objectForKey:@"entity"] objectForKey:@"user"] objectForKey:@"integral"];
                                    [[dataBase share]updatePrimaryInforWithModel:model];
                                    NSDictionary*dict=@{@"value":@"100"};
                                    [[NSNotificationCenter defaultCenter]postNotificationName:@"showIncreaImage" object:nil userInfo:dict];
                                }
                            }
