...
-(void)initDate{
    ...
    if ([model.userPost integerValue]==2||[model.userPost integerValue]==3) {
        secondArray=@[@"我的服务"];
    }
    NSArray*normalAdressArray=@[@"常用地址"];
...
//    [_dataArray addObject:thirdArray];
    [_dataArray addObject:normalAdressArray];
...
-(UITableViewCell*)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    loginModel*logModel=[[dataBase share]findLoginInformationWithID:delegate.id];
       if (indexPath.section==0) {
        ...
        cell.integrity.text=[NSString stringWithFormat:@"%u%%",[logModel.integrity intValue]];
...
-(CGFloat)tableView:(UITableView *)tableView heightForHeaderInSection:(NSInteger)section{
    ...
    loginModel*model=[[dataBase share]findLoginInformationWithID:delagate.id];
    if (section==0) {
        return 0;
    }
    
    if ([model.userPost integerValue]!=1) {
        if (section==1||section==4||section==8) {
            return 20;
        }
        
    }else{
    
    if (section==1||section==3||section==7) {
        return 20;
        }
    }
    return 0.5;
