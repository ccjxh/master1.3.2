          ...
          //integral
            ...
            if ([[[dict objectForKey:@"entity"] objectForKey:@"user"] objectForKey:@"integral"]) {
                if ([model.integral integerValue]-[[[[dict objectForKey:@"entity"] objectForKey:@"user"] objectForKey:@"integral"] integerValue]>0) {
                    ...
                    NSDictionary*parent=@{@"value":[NSString stringWithFormat:@"%u",[model.integral integerValue]]};
    ...
    [[httpManager share]POST:urlString parameters:dict success:^(AFHTTPRequestOperation *Operation, id responseObject) {
        ...
        if ([[objDic objectForKey:@"rspCode"]integerValue] == 200)
        {
            ...
            if ([[[objDic objectForKey:@"entity"] objectForKey:@"user"] objectForKey:@"integrity"] ) {
                model.integrity=[[[objDic objectForKey:@"entity"] objectForKey:@"user"] objectForKey:@"integrity"];
            }
            if ([[[dict objectForKey:@"entity"] objectForKey:@"user"] objectForKey:@"integral"]) {
                if ([model.integral integerValue]-[[[[dict objectForKey:@"entity"] objectForKey:@"user"] objectForKey:@"integral"] integerValue]>0) {
                    model.integral=[[[dict objectForKey:@"entity"] objectForKey:@"user"] objectForKey:@"integral"];
                    NSDictionary*parent=@{@"value":[NSString stringWithFormat:@"%lu",[model.integral integerValue]]};
                    NSNotification*noction=[[NSNotification alloc]initWithName:@"showIncreaImage" object:nil userInfo:parent];
                    [[NSNotificationCenter defaultCenter]postNotification:noction];
                }
            }
