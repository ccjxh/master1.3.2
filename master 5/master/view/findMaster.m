...
-(void)customUI{
    ...
    loginModel*model=[[dataBase share]findLoginInformationWithID:delegate.id];
    ...
    if ([model.signState integerValue]==1) {
        
        [_opinionButton setTitle:[NSString stringWithFormat:@"明日签到+%u",[model.nextDayIntegral integerValue]] forState:UIControlStateNormal];
    }
    else {
    
        [_opinionButton setTitle:[NSString stringWithFormat:@"今日签到+%u",[model.todayIntegral integerValue]] forState:UIControlStateNormal];
...
-(void)reloadData{
    ...
    dispatch_async(dispatch_get_main_queue(), ^{
        ...
        _timeLabel.text=[NSString stringWithFormat:@"您已经连续签到了%u天",[logModel.renewDay integerValue]];
        if ([logModel.signState integerValue]==1) {
            [_opinionButton setTitle:[NSString stringWithFormat:@"明日签到+%u",[logModel.nextDayIntegral integerValue]] forState:UIControlStateNormal];
            _opinionButton.userInteractionEnabled=NO;
            [_opinionButton setTitleColor:[UIColor lightGrayColor] forState:UIControlStateNormal];
        }
        else {
            [_opinionButton setTitle:[NSString stringWithFormat:@"今日签到+%u",[logModel.todayIntegral integerValue]] forState:UIControlStateNormal];
            _signedButton.userInteractionEnabled=YES;
        }
        _scoreLabel.text=[NSString stringWithFormat:@"%u",[logModel.integral integerValue]];
    });
