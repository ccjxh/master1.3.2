...
@implementation peopleDetailTableViewCell
{
    ...
    __weak IBOutlet NSLayoutConstraint *pageviewWidth;
    __weak IBOutlet UILabel *expenienceLabel;
    __weak IBOutlet NSLayoutConstraint *ageWidth;
}
- (void)awakeFromNib {
    // Initialization code
    [self customUI];
...
-(void)customUI{
    ...
    expenienceLabel.textColor=COLOR(14, 148, 231, 1);
    _backview.layer.borderColor=expenienceLabel.textColor.CGColor;
    _backview.layer.borderWidth=2;
    _backview.layer.cornerRadius=10;
    _master_workExperience.textColor=expenienceLabel.textColor;
...
-(void) upDateWithModel:(personDetailViewModel *)model
{
    ...
    [self.master_headImage sd_setImageWithURL:url placeholderImage:[UIImage imageNamed:headImageName] completed:^(UIImage *image, NSError *error, SDImageCacheType cacheType, NSURL *imageURL) {
        if (error) {
            self.master_headImage.image=[UIImage imageNamed:@"碎图"];
        }
    } ];
...
//    self.master_workExperience.textColor=COLOR(42, 42, 42, 1);
    ...
    ageWidth.constant=self.master_age.text.length*14-10;
    //工作经验
    ...
    //更新时间
    self.refreshLabel.text =[NSString stringWithFormat:@"%ld",model.pageView];
    //浏览量
    self.master_orderCount.text =model.service[@"refreshed"];
    pageviewWidth.constant=self.refreshLabel.text.length*14;
    //认证方式
